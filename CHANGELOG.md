...
为与线代侧对称（线代页标题已带「线性代数」前缀），给高等数学的两个工具页标题
加上科目名：

| 文件 | 原 | 现 |
|---|---|---|
| `cheatsheet.html` | 公式速查表 | 高等数学公式速查表 |
| `exam.html` | 综合自测卷（考研数一难度） | 高等数学综合自测卷（考研数一难度） |

同时同步各自 `<title>` 标签。**只改标题文字，未动正文、公式、数据与布局**；
侧栏与顶栏导航的入口标签（「公式速查表」「综合自测卷」）保持原样，
与线代侧一致。

确认：这两个工具页的 `<h1>` 是手写在 HTML 里的（`gen_pagehead` 只处理章节页，
工具页无 `SHELL:PAGEHEAD` 标记），因此改 HTML 不会被 build-shell 覆盖。

验证：11 项校验全绿；203 项单测全绿；图表自检通过。


## v0.9.3 — 2026-09-20

### 线代专属工具页：公式速查表与综合自测卷（第 39 节）

#### 问题

切到线性代数后点「公式速查表」「综合自测卷」，内容仍与高数完全一样。

#### 根本原因（数据源未隔离 + 导航未隔离）

1. **数据源未隔离**：`subjects.calculus.tools` 与 `subjects.linearAlgebra.tools`
   **指向同一对文件** `cheatsheet.html` / `exam.html`，而这两个文件里是高数内容硬编码。
2. **导航未隔离**：外壳的顶栏/侧栏工具链接由 `data['tools']` 生成，线代页此前
   指向的仍是高数的 `../cheatsheet.html` / `../exam.html`。

**不是缓存问题**：静态站无 Keep-Alive 或页面级缓存，每次都是重新加载文件；
串内容的直接原因就是两科目共用同一对文件。

#### 修法（内容取自线代源站，未编造）

| 文件 | 改动 |
|---|---|
| `linear-algebra-cheatsheet.html` | **新增**：由线代源站 `cheatsheet.html`（43 KB）迁移，含一至六章公式（行列式、矩阵运算、矩阵的秩、线性方程组、向量组线性相关性、特征值与二次型） |
| `linear-algebra-exam.html` | **新增**：由线代源站 `exam.html`（66 KB）迁移，含客观题与计算证明题 |
| `assets/course-data.json` | `linearAlgebra.tools` 改指上述两个专属文件 |
| 线代 9 个页面外壳 | 重建顶栏/侧栏/底栏，工具链接改指线代专属页 |
| `tests/check-html.py` | 清单纳入两个新页 |

迁移沿用线代章节页同一套流程：换本地 MathJax、套统一外壳、分片排版调度、
`.bi-en` → `.en` 与无标记英文段标记（中英切换）、`en-line` → `q-stem-en`。

#### 顺带修掉的排版缺口

工具页有表格与折叠块，结构比章节页不规则：初版分别有 1 处与 9 处公式未渲染
（`div.en`、`td`、`strong`）。**试过把 `.tbl-wrap` / `.reveal-body` / `.en` 加进选择器，
反而更糟**（去嵌套过滤把 `td` 排除了，未渲染升到 104 处）。
最终改为：选择器不动，在重扫次数用尽后**整块兜底排版一次**——
只在有残留时才付这一次全量代价。四个工具页现均 0 未渲染。

#### 验证

| 项 | 结果 |
|---|---|
| 内容隔离 | ✓ 高数 `公式速查表` / `综合自测卷（考研数一难度）`；线代 `线性代数公式速查表` / `线性代数综合自测卷`，`data-subject=linearAlgebra` |
| 公式渲染 | ✓ 四页 0 未渲染（线代速查 377、线代自测 802、高数速查 528、高数自测 595） |
| 横向溢出 | ✓ 四页均为 0 |
| 导航一致性 | ✓ 线代 9 个页面的顶栏/侧栏工具链接全部指向线代专属页 |
| 站点校验 11 项 | ✓ 全绿 |
| `check-html` | ✓ **25/25 页**、694 链接 0 失效、0 失效锚点 |
| JS 单测 6 套（203 项） | ✓ 全绿 |
| 图表视觉自检 | ✓ 通过 |
...
#### 视频：BV1Et421E7jk（一高数）分P 一一对应

分P 数据取自 B 站官方接口 `x/player/pagelist`，**不手工编造**。
22 个分P 中 P1 是「一高数指南」，**P2–P22 与 6 章 21 节一一对应**：

| 节 | P | 时长 | 节 | P | 时长 |
|---|---|---|---|---|---|
| 1-1 | 2 | 19:59 | 4-1 | 15 | 29:20 |
| 1-2 | 3 | 20:28 | 4-2 | 16 | 29:16 |
| 1-3 | 4 | 23:29 | 4-3 | 17 | 34:02 |
| 1-4 | 5 | 25:17 | 5-1 | 18 | 43:52 |
| 1-5 | 6 | 31:13 | 5-2 | 19 | 21:09 |
| 1-6 | 7 | 45:19 | 6-1 | 20 | 25:47 |
| 2-1 | 8 | 39:34 | 6-2 | 21 | 20:40 |
| 2-2 | 9 | 22:56 | 6-3 | 22 | 11:46 |
| 2-3 | 10 | 9:22 | 3-1 | 12 | 6:39 |
| 2-4 | 11 | 11:18 | 3-2 | 13 | 61:34 |
| 3-3 | 14 | 32:12 | | | |

实现：主表新增一条 BV；MAP 并入 21 条；6 页的 `<h2>` 补 `data-video-map`\
`data-video-title`，并在其后插入 `data-video-entry` 占位。
六章实测视频按钮 **21 个**、链接形如 `…BV1Et421E7jk?p=N`、空占位 0。

#### 中英切换：线代页接入（源文为中英混排）

线代源文的英文容器是 `.bi-en`（高数是 `.en`/`.zh`），且**639 个英文段根本没容器**。
三步适配：
1. 容器 `.bi-en` → `.en`（从而被 `[data-lang="zh"] .en` 隐藏）
2. **无标记英文段**：用流的线性遍历（不递归、避免非贪婪正则错配嵌套块），
   把中文段之后的英文 `<p>`/`<li>` 标记为 `.en`
3. 测验英文类名 `en-line` → `q-stem-en`（与高数等价，校验器与 CSS 均已覆盖）

`style.css` 另补 `.bi-en-label` / `en-term` / `bi-en-term` 的隐藏规则。
实测：中英模式下 167 个 `.en` 全部可见；**纯中文模式下可见 0 个**。

#### 校验器支持多科目（不误报、不放过）

| 文件 | 改动 |
|---|---|
| `tests/check-bilingual.py` | `parse_course()` 纳入 `subjects[]` 其余科目的章节（与 site.js 同源口径），否则线代小节的视频条目会被误报为「不在 COURSE 里」 |
| 同上 | `ZH_MODE_ALLOW` 补线代专有名词/人名/缩写：Schur、Sylvester、Hessian、Schmidt、Jordan、Campbell、Mises、PCA、Hamilton、Cayley、Gram、Vandermonde，以及中文行文里作括号注释的术语（eigenvalue/matrix/orthogonal/determinant/adjugate/commute/rank/bladed disk/von Neumann） |

这两处是**校验器与站点的口径对齐**，不是放宽标准：漏出的英文仍会被抓
（本轮正是靠它抓出 639 个无标记英文段）。

#### 修掉的缺陷

1. **线代视频条目插错位置**：我最初把 21 条 MAP 条目插进了 `window.__VIDEO__` 导出对象，
   导致 `hasVideo('la-s1-1')` 为 false、按钮全显示「暂无视频」。已改为按 MAP 大括号配平定位。
2. **节 id 碰撞**：线代节 id 与高数同名（都叫 `s1-1`），MAP 以节 id 为键会串；
   且 `h2[id]` 与数据 `sections` 必须一致，否则「已掌握」按钮不注入（实测为 0 个）。
   现统一加 `la-` 前缀。
3. **合并脚本漏 `</body>`**：`check-html` 报「22 页中 16 页通过」，已在脚本里补上。

#### 验证

| 项 | 结果 |
|---|---|
| 站点校验 11 项 | ✓ 全绿（含 check-bilingual） |
| `check-html` | ✓ 22/22 页、624 链接 0 失效、0 失效锚点 |
| JS 单测 6 套（203 项） | ✓ 全绿 |
| 布局 / 对比度 / 图表视觉自检（46 图） | ✓ |
| 六章渲染 | ✓ 公式 2405/2771/2071/2118/2129/1540 全渲染、未渲染 0、溢出 0 |
| 视频按钮 | ✓ 六章合计 21 个、链接正确、空占位 0 |
| 中英切换 | ✓ 纯中文模式下英文可见数 0 |
| 「已掌握」 | ✓ 6/4/3/3/2/3 = 21 个 |
| 题数 | ✓ 75/53/46/45/34/47 = 300，与源项目一致 |

#### 仍未做

- 线代的 `index.html` / `cheatsheet.html` / `exam.html` **尚未并入**（六章正文已完成）
- 本轮未重出安装包（待下一轮）
